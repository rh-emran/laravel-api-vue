            ...
            <div class="border-l-4 border-blue-500 pl-4 mt-12">
                ...
                <div v-if="authStore.user && authStore.user.id === post.user_id" class="flex items-center gap-6 mt-6">
                    <form @submit.prevent="deletePost(post)">
                        <button class="text-red-500 font-bold px-2 py-1 rounded-sm border border-red-300">Delete</button>
                    </form>

                    <RouterLink :to="{ name: 'update', params: { id: post.id } }" class="text-green-500 font-bold px-2 py-1 rounded-sm border border-green-300">Update</RouterLink>
                </div>
            </div>
        </div>
